...
- **ウォッチリストが空の場合**：接続は維持したまま、30秒ごとに `：keepalive` コメントのみ送信する。価格データは送信しない。銘柄が追加されると次のポーリングサイクル（約500ms）から自動的に価格が流れ始める。
...
2. `chat_messages` テーブルから最近の会話履歴を読み込む（`ORDER BY created_at DESC LIMIT 20` で直近20件を取得後に昇順に並び替え、10往復分）。
